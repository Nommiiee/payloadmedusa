...
//     redirectUrl = `${request.nextUrl.origin}/store/${countryCode}${redirectPath}${queryString}`
...
// export const config = {
//   matcher: [
//     '/store/((?!api|_next/static|_next/image|favicon.ico|images|assets|png|svg|jpg|jpeg|gif|webp|admin).*)',
//   ],
// }
...
async function getCountryCode(
  ...
  try {
    ...
    const urlCountryCode = urlSegments[2]?.toLowerCase() // 👈 after /store
...
export async function middleware(request: NextRequest) {
  ...
  const urlSegments = request.nextUrl.pathname.split('/')
  const urlCountryCode = urlSegments[2] // 👈 /store/<countryCode>/...

  const urlHasCountryCode = countryCode && urlCountryCode === countryCode

  // if /store already has the correct country code
  if (urlHasCountryCode && cacheIdCookie) {
    return NextResponse.next()
  }

  // if /store has country code but no cookie -> set cookie
  if (urlHasCountryCode && !cacheIdCookie) {
  ...
  // redirect to /store/:countryCode
  if (!urlHasCountryCode && countryCode) {
    const restPath = urlSegments.slice(2).join('/') // everything after /store/
    const queryString = request.nextUrl.search || ''
    const redirectUrl = `${request.nextUrl.origin}/store/${countryCode}${
      restPath ? '/' + restPath : ''
    }${queryString}`
